...
def get(path):   #把一个函数映射成一个URL处理函数
	'''
...
def post(path):     #把一个函数映射成一个URL处理函数
	'''
...
def get_required_kw_args(fn):	#收集没有默认值的命名关键字参数
	args = []
	paramas = inspect.signature(fn).parameters
	for name, param in paramas.items():
...
class RequestHandler(object):    #从URL中分析出要接收的参数，从request中获取参数，调用url处理函数，返回response对象
...
def add_route(app, fn):   #注册一个url处理函数
