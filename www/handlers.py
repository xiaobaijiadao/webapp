...
async def authenticate(*, email, passwd):
	...
	logging.info('page pwd: %s' % passwd)
	user = users[0]
	logging.info('db pwd: %s' % user.passwd)
...
async def api_create_blog(request, *, name, summary, content):
	...
	blog = Blog(user_id=request._user_.id, user_name=request._user_.name, user_image=request.__user__.image, name=name.strip(), summary=summary.strip(), content=content.strip())
	await blog.save()
	return blog
